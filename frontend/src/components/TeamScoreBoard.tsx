...
export default function TeamScoreBoard({ teamAName, teamBName, teamStats, progress }: Props) {
  ...
  return (
    <div className="rounded-2xl shadow-xl overflow-hidden mb-4"
      ...
      {/* Header bar */}
      <div className="flex items-center justify-center gap-2 py-2 border-b border-amber-500/20 flex-wrap px-2"
        style={{ background: 'rgba(212,175,55,0.12)' }}>
        <span className="text-amber-400 text-xs font-black tracking-widest uppercase">★ BADMINTON STARS ★ Live Scoreboard</span>
        <span className="hidden sm:inline text-amber-300/50 text-xs">·</span>
        <span className="hidden sm:inline text-amber-300/70 text-xs">20 pts / match win &nbsp;|&nbsp; 10 pts / player</span>
      </div>

      {/* Main score row */}
      <div className="flex items-center justify-between px-3 sm:px-6 py-3 sm:py-5 gap-2">
        {/* Team A */}
        <div className={`flex-1 text-center transition-transform duration-300 ${aLeading ? 'scale-105' : ''}`}>
          <div className="text-xs font-black uppercase tracking-wider mb-2" style={{ color: '#00BFFF' }}>{teamAName}</div>
        ...
        <div className="px-3 text-center flex-shrink-0">
          ...
          {aLeading && <div className="text-xs mt-1 font-black" style={{ color: '#00BFFF' }}>Leading ↑</div>}
          {bLeading && <div className="text-xs mt-1 font-black" style={{ color: '#FFD700' }}>Leading ↑</div>}
        ...
        <div className={`flex-1 text-center transition-transform duration-300 ${bLeading ? 'scale-105' : ''}`}>
          <div className="text-xs font-black uppercase tracking-wider mb-2" style={{ color: '#FFD700' }}>{teamBName}</div>
      ...
      <div className="px-3 sm:px-5 pb-3 sm:pb-4">
        <div className="flex rounded-full overflow-hidden h-2.5 bg-slate-700 mb-1.5">
          <div className="transition-all duration-700 ease-out" style={{ width: `${aPct}%`, background: 'linear-gradient(90deg,#0078A8,#00BFFF)' }} />
          <div className="transition-all duration-700 ease-out" style={{ width: `${bPct}%`, background: 'linear-gradient(90deg,#B8860B,#FFD700)' }} />
        </div>
        <div className="flex justify-between text-xs">
          <span className="font-black" style={{ color: '#00BFFF' }}>{aPct}%</span>
          <span className="text-slate-400">
            {progress.completed} / {progress.total} matches played
          </span>
          <span className="font-black" style={{ color: '#FFD700' }}>{bPct}%</span>
        </div>
      </div>
    </div>
  );
